...
void upgrade_gradient_single_point(double *X, double *G, double *F, int N, int K, int D_x,  int N_all, int C,
                                    int c_class, int low, int up, int n){
            
 
            double dz_dbeta;
            double val[K];            
            double local_grad_beta[K];
            int ptr;                    
            int N_x = up - low + 1;                                    
            ...
            get_softmax_generic(K, N_x, F);
                            
            for (int ll=0; ll< K; ll++) local_grad_beta[ll] = 0;
            ...
            for (int k = 0; k < K; k++){      // for each z_k there is a different gradient 
                ...
                for (int l = 0 ; l < K; l++){                                                                        
                    ...
                    for (int m = 0; m < N_x; m++){  // sum over all the x in S_m                                                            
                           if (l==k){
                            val[l] = val[l] + 
                                    F[m*K+k]*(F[m*K+k]-1)*( G[m*K+k] );
                           }else{
                            val[l] = val[l] + 
                                       F[m*K+k]*F[m*K+l]*( G[m*K+l] );
                           }
                    }      // m             
                }  // l
                ...
                for (int ll=0; ll< K; ll++){ 
                    dz_dbeta = val[ll]; 
                    dz_dbeta  = dz_dbeta/N_x; // Normalized codebook
                    local_grad_beta[ll] = local_grad_beta[ll] + dL_dz*dz_dbeta;
            ...
            // Update global grad_beta
            pthread_mutex_lock(&mutexsum);
            for (int ll=0; ll< K; ll++){ 
                    grad_beta[ll] = grad_beta[ll] + local_grad_beta[ll];
...
handle_request(struct request* a_request, int thread_id)
{
    if (a_request) {
     ...
     upgrade_gradient_single_point(td_array[n].X, td_array[n].G, td_array[n].P, td_array[n].N, 
                td_array[n].K, td_array[n].D_x, 
...
void *call_update_gradient(void *threadarg){
    ...
    upgrade_gradient_single_point(td->X, td->G, td->N, td->K, td->D_x,  
...
{    
    ...
    int NTHREADS, N,  K,  D_x,    N_all,  C, n, t; 
    double *low, *up;
    void *status;
    int rc;    
    ...
    grad_beta    = mxGetPr(plhs[0]); 
    
    
    // mexPrintf("K=%d\n",K);
    //for (int k = 0; k < K; k++) mexPrintf("beta[%d]=%.3f\n",k, beta_x[k]);
   ...
   for (n = 0; n < N; n++){                
       ...
       td_array[n].P       = (double *) mxMalloc(N_x * td_array[n].K * sizeof(double)); 
