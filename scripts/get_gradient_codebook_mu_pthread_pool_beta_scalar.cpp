  ...
  */
   double maxi[N_x];
    for (j = 0; j < N_x; j++){ /* determines maximum for each colum */
        i = 0; PP(i,j)  = - beta_x *  PP(i,j);
        maxi[j] = PP(i,j);
        for (i=1; i < K; i++){
            PP(i,j) = - beta_x *  PP(i,j); /* updates P(i,j) before using it */
                ...
                for (int l = 0 ; l < K; l++){                                       
                    ...
                    for (int m = 0; m < N_x; m++){  // sum over all the x in S_m                        
                        ...
                        for (int d=0; d < D_x; d++){                  
                           ...
                           if (l==k){
                            val[ptr] = val[ptr] + 
                                        G[m*K+k]*(1-G[m*K+k])*( X[d*N_x+m] - Mu[d*K +k] );
                           }else{
                            val[ptr] = val[ptr] + 
                                       G[m*K+k]*G[m*K+l]*( Mu[d*K +l] -X[d*N_x+m]);
                           }
                        }   // d         
                    }      // m            
                }  // l
