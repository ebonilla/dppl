function [centres, options, post, errlog, n] = mykmeans(centres, data, options)
...
% same as in netlab except that the squared distances are computed
% using sq_dist and 
% Modified by ebonilla to return the number of actual iterations spent
...
for n = 1:niters
  ...
  d2 = sq_dist(data', centres');   % Calculate posteriors based on existing centres
  %d2 = dist2(data, centres);

  
  [minvals, index] = min(d2, [], 2);  % Assign each point to nearest centre
